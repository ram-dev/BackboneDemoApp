...
if(options.stickerType== null){
		...
		this.mousedown(function(event){
			if (event.target == image) {
				...
				var element = annotationCallback(options.stickerType);
		
				$(this).append(element);
			
			  var stickerCount=$(this).find('.sticker').length;

				var slideNumber= $(this).data('slide-number');

				$('#carousel-selector-'+ slideNumber).find('span').text(stickerCount);
	...
	$.fn.seralizeAnnotations = function(xPosition, yPosition) {
		var annotations = [];
		this.each(function(){
			annotations.push({imageId: $(this).attr("imgId"),type: $(this).attr("class"), feedback: $(this).find('textarea').val(), x: $(this).relativeX(xPosition), y: $(this).relativeY(yPosition), response_time: $(this).data('responseTime')});
		});
		return annotations;
	};
